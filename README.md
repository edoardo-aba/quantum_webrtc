## Notes
- Up to now the Implementation works perfectly on Chrome as well on Firefox, but other browsers may have stricter security requirements for accessing getUserMedia over HTTPS. A self-signed certificate might be rejected while on others being accepted e.g Chrome (once manually accepted).
